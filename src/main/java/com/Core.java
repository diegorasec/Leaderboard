...
public class Core {
	...
	public static void criaLeaderboard(String filePath, Map<Categoria, List<Equipe>> lead) {
		...
		while (iterator.hasNext()) {
			...
			/*
			 * htmlContent.append("<div id=\""+categoria.getNome()
			 * +"\" style=\"display:block\";>");
			...
			for (Equipe e : equipeOpt) {
				...
				if (pointsLast >= 0 && positionLast >= 0 && e.getPoints().equals(pointsLast)) {
					...
					if (equipe.equalsIgnoreCase("laranja")) {
						htmlContent.append("<table style=\"width: 70%;  border-collapse: collapse; border: none;\">"
								...
								+ ".png\" alt=\"Imagem Equipe\" style=\"width: 140px; height: 100px; margin-bottom: 0px;\">"
								+ "</td>" + "<td style=\"vertical-align: top; padding-left: 10px;\">");

					} else {
						htmlContent.append("<table style=\"width: 70%;  border-collapse: collapse; border: none;\">"
								+ "<tr>" + "<td style=\"width: 100px; text-align: center; vertical-align: top;\">"
								+ "<img src=\"src/main/resources/img/" + equipe
				...
				for (Result r : e.getResult()) {
					...
					if (r.getPosition().equals(1)) {
						...
						htmlContent.append(" <td style=\"color: #fff; font-size: 16px;\">(" + r.getPosition() + ") "
								+ r.getResult() + " " + kgs + "</td>\n");
				...
				if (line.length > 0 && categId.equals(Integer.valueOf(line[2]))) {
					...
					try (CSVReader r = new CSVReader(new FileReader(PATH_RESOURCES + "results.csv"))) {
						...
						while ((l = r.readNext()) != null) {
							for (Wod w : foundWods) {
								if (w.getId().equals(wodNum) && !l[wodNum + 1].isEmpty()
